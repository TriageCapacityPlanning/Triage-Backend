...
class DataFrame:
    ...
    def get_sample(self, interval: int, triage_class: int):
        ...
        base_referal_prediction = self.predictions[interval][0] * \
            self.distribution[triage_class]
        # Generate a variance of referals relative to a triage class
        variance = random.uniform(-self.predictions[interval][1],
                                  self.predictions[interval][1]) * self.distribution[triage_class]
